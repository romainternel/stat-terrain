...
### Disposition (rappel du Design, contrainte visuelle)
3 boutons sur la première rangée (`BUT`/`ARRÊT`/`NON CADRÉ`), 2 sur la seconde (`PB`/`JET FRANC`) — disposition `.act-h` existante, `flex:1`, gap `5px`, strictement identique à aujourd'hui pour un seul bloc. Le gain visuel vient uniquement de la suppression du second bloc (grisé) en dessous, pas d'un redimensionnement des boutons existants — ne pas agrandir les boutons pour "combler" l'espace libéré, garder les proportions actuelles déjà validées sur iPhone (STORY-24).
